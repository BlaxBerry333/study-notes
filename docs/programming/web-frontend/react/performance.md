...
```tsx
const UserBadge = React.memo(function UserBadge({ name, level }: {
  name: string
  level: number
}) {
  return <span>{name} (Lv.{level})</span>
})
